...
```{r}
#Exercise 2:
EPAair.subsample$Year <- as.factor(EPAair.subsample$Year)
ozone.year <- ggplot(EPAair.subsample, aes(x = Ozone, fill = Year)) +
  labs(x = "Ozone AQI value", y = "Density", title = "Ozone by year") +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values=c('blue','yellow'))
ozone.year

ozone.box <- ggplot(EPAair.subsample, aes(x = Year, y = Ozone)) +
  geom_boxplot() +
...
ozone.violin <- ggplot(EPAair.subsample, aes(x = Year, y = Ozone)) +
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom = "text", fun.y = max, vjust = -1, size =4,
               label = c("b","a"), color = 'red') +
  labs(x = "", y = "Ozone AQI Value")
ozone.violin
```
...
OzonePM25lm <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(OzonePM25lm)
...
library(corrplot)
correlation <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())
correlation
