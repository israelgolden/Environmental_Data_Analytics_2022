...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #Peter Lake OR Paul Lake
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # cleaner way of filtering dataset
...
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) # every time daynum matches what is in the vector, that will be extracted for you.
...
NTl.phys.data.challenge <- filter(NTL.phys.data, lakename %in% 'Tuesday Lake' & year4 %in% c(1990:1999))
...
# the colon here selects all columns between sample date and temperature_C
# creates a tidier dataset
```
### Mutate
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% # start from original data, filter by lake name
