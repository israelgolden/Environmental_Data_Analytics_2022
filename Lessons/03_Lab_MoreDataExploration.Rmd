...
3. Date objects
4. Note Include stringsAsFactors = True when importing files
...
# dataframe1 <- data.frame("ID" = d, "Color" = e, "P" = f)
...
```{r}
highs <- c(58, 41, 43, 45, 35, 42, 49, 52, 56, 66)
lows <- c(31, 19, 25, 27, 18, 23, 25, 29, 43, 49)
```
...
```{r}
# You could do it with straight up clunky math - but this wastes time!!!
high_c <- (highs-32)*(5/9)
low_c <- (lows-32)*(5/9)

# OR write a function (FASTER!)

temp_converter_FtoC <- function(x){
  celsius_temp <- (x-32)*(5/9)
  print(round(celsius_temp,3))
}

high_c <- temp_converter_FtoC(highs)
low_c <- temp_converter_FtoC(lows)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
Durham_forecast_df<- data.frame(highs_f = highs, 
                                lows_f = lows, 
                                highs_c = high_c, 
                                lows_c = low_c)
Durham_forecast_df
colnames(Durham_forecast_df) <- c("highs (F)","lows (F)","highs (C)","lows (C)")
rownames(Durham_forecast_df) <- c("1/25", "1/26", "1/27", "1/28", "1/29", "1/30", "2/1", "2/2", "2/3", "2/4")
```
...
```{r}
summary(Durham_forecast_df)
summary(Durham_forecast_df$`highs (F)`)
sd(Durham_forecast_df$`highs (F)`)
```
...
# if it's organized month, day, year (01/27/2022) used the mdy() function
date_obj_today
