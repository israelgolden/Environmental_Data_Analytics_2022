...
> Add notes here: Temperature, dissolved oxygen, and light availability in lakes are all important variables for understanding lake ecosystems and health. For example, if there are higher temperatures, there will be less dissolved oxygen in water which has implications for viability of fish species. Similarly, more light availability in lakes could encourage the growth of aquatic plants like algae, that, upon blooming and decaying, soak up dissolved oxygen. 
...
PartyLikeIts1999 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
TuesdayLakeInThe90s <- NTL.phys.data %>%
  filter(year4 %in% c(1990:1999)) %>%
  filter(lakename == "Tuesday Lake")

# alternative

Exercise2 <- filter(NTL.phys.data,year4 %in% (1990:1999) & lakename == "Tuesday Lake")
 ```
Question: Why don't we filter using row numbers?

> Answer: Row numbers are arbitrary - locations of the data rather than attributes in their own right.
...
JulysInTuesdayLake.90s <- NTL.phys.data %>%
...
July.90s.Surface.Data <- JulysInTuesdayLake.90s %>%
  filter(depth == 0) %>%
...
Exercise5 <- 
  pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, values_to = "irradiance")

#Exercise 6: spread temperatureC into more than one column based on the depth
Exercise6 <- pivot_wider(NTL.phys.data, names_from = temperature_C, values_from = depth)
```
