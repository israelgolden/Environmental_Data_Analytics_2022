...
> Answer: There's a big readout showing which packages have been installed.
...
> ANSWER: You can find this file in the metadata folder within the data folder in the project.The placement of this file fulfills best practices for reproducible data anlaysis by being carefully organized in its own metadata folder within the data folder, a logical location. The information found in this file relate to the best practices for reproducible data analysis by explaining what the data is, how it was collected, when it was collected, and who to contact for questions.
...
> Answer: Time series, you never remove NAs, but you can replace NAs with another value. You would remove NAs if you wanted to calculate the mean or any summary stats. 

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #takes dataframe we want to remove NAs from - new object has no NAs. There are only 5342 observations compared to 33,690
dim(USGS.flow.data)
...
# This is saying, if the date is less than 191226 (12/19/26) make it 19, else make it 20
# the paste0 glues that resulting 19 or 20 to our d, which is the input in the datetime
# column
...
# now convert it back to dates with the as.Date function
